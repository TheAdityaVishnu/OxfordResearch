...
from ForMFCC3 import *
#mat3 = scipy.io.loadmat('Test819.mat');
#Testing = mat3['Testing'];
#Class_Testing = mat3['Class_Testing'];
beta = 0.001
#from NewTraining_TUM import *
#Testing = np.ascontiguousarray(Testing.T);
print('in main')
print(Testing.shape)
print(Training.shape)
print(Class_Training.shape)
print(Class_Testing.shape)

#from Draft1 import *
#from ForDelta import *

#from ForScaling import *
...
with tf.name_scope('convolution1'):
    ...
    x_image = tf.reshape(x, [-1, 98, 14, 1])
...
#loss2 = tf.reduce_mean(cross_entropy + beta * regularizer)
...
#regularizer = l1_regularizer(W_fc2)
...
with tf.Session() as sess:
    ...
    for g in range(0,1000):
        Global_Data = np.arange(0, 13535)
        print('<---------Epoch Number: %d -------------> '% g)
        for i in range(337):
            batch1, batch2 = next_batch(40, Training, Class_Training)
            if i % 5 == 0:
                #train_accuracy = accuracy.eval(feed_dict={x: batch1, y_: batch2, keep_prob: 1.0})
                train_summary , cost, acc  = sess.run([summaryMerged, cross_entropy, accuracy], feed_dict = {x: batch1, y_: batch2, keep_prob:1.0})
                writer.add_summary(train_summary, k)
                k = k+5
                print('step %d, training accuracy %g loss is %g' % (i, acc, cost))
            train_step.run(feed_dict={x: batch1, y_: batch2, keep_prob: 0.5})
        #with open(input_graph_path,'wb') as f:
        #    f.write(sess.graph_def.SerializeToString())
        test = accuracy.eval(feed_dict={
            x: Testing, y_: Class_Testing, keep_prob: 1.0})
        print('test accuracy %g' % test)
        tf.summary.scalar('Testing accuracy', test)
